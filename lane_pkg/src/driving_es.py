...
global lower_white
lower_white = np.array([160,190,210])
global upper_white
upper_white = np.array([215,235,250])


global lower_yellow
lower_yellow = np.array([50,140,190])
global upper_yellow
upper_yellow = np.array([80,160,210])

# # 강의 yello
# global lower_yellow
# lower_yellow = np.array([15,128,0])
# global upper_yellow
...
def color_filter_hsv(img):
        ...
        lower_lane = np.array([0, 0, 0]) #0 46 170 / white lane: 0 0 126
        upper_lane = np.array([255, 255, 255]) #79 255 255 / white lane : 255, 64, 180


        masked = cv2.inRange(hsv_image, lower_lane, upper_lane)
...
def bird_eye_view(img, width, height):
    ...
    # src = np.float32([[220, 296],
    #               [448,296],
    #               [0,440],
    #               [width,440]])

    # dst = np.float32([[0,0],                   #넓
    #               [640,0],
    #               [140,480],
    #               [500,480]])
    dst = np.float32([[140,0],                     #좁
                  [500,0],
                  [220,480],
                  [420,480]])  
    ...
    #src, dst는 모두 순서대로 왼쪽위, 오른쪽위, 왼쪽아래, 오른쪽 아래 점입니다.230, 415
...
def start():
    ...
    image_sub = rospy.Subscriber("/image_jpeg/compressed",Image,img_callback)
