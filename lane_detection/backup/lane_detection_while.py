    ...
    def __init__(self):
        ...
        while not rospy.is_shutdown():
            ...
            # if len(self.img) != 0:
                ...
                if (12 <= self.status_msg_x) and (self.status_msg_y <= -2.5) and (self.status_msg_heading >= 45):
                    self.stopline_cnt = 3
                ...
                if self.stopline_cnt == 0:  # Finished
                    ...
                    #--------------------------------------------- 장애물 ---------------------------------------------#
                    if (0.5 < self.obstacle_dynamic_static_x < 1.5) and self.finish_detection == False:
                        ...
                        if len(self.obstacle_y_list) == 400 and self.finish_detection == False :
                            ...
                            if abs(self.obstacle_y_list[150] - self.obstacle_y_list[-3]) > 0.08:
                                self.is_dynamic= True
                                self.is_static = False
                    ...
                    if self.is_dynamic:
                        if -0.45 < self.obstacle_dynamic_static_y < 1.0 and not (self.obstacle_dynamic_static_x == 0 and self.obstacle_dynamic_static_y == 0):
                            self.publishMotorSteerMsg(0, self.steer_msg)
                ...
                # ########################################## 3 3 3 3 3 ##########################################
                    ...
                    print("Mode: 3")
                    # print("Yaw:", self.status_msg_heading)
                    print("Lane:",self.current_lane)
                    # print("Passed:", self.is_static_ob_passed)
                    self.sliding_window_select_line = 'Right'
                    ...
                    if (0.1 < self.obstacle_dynamic_static_x < 1.5) and self.finish_detection == False:
                        ...
                        if len(self.obstacle_y_list) == 400 and self.finish_detection == False :
                            ...
                            if abs(self.obstacle_y_list[150] - self.obstacle_y_list[-3]) > 0.08:
                                self.is_dynamic= True
                                self.is_static = False
                                print("동적확정 y delta: ", abs(self.obstacle_y_list[3] - self.obstacle_y_list[-3]))
                            # elif (-0.15 <= self.obstacle_y_list[0] <= 0.15 and -0.15 <= self.obstacle_y_list[-1] <= 0.15):
                            else:
                                self.is_static = True
                                self.is_dynamic = False
                    ...
                    if self.is_dynamic:
                        ...
                        if self.current_lane == 'RIGHT':
                            ...
                            if -0.2 < self.obstacle_dynamic_static_y < 0.2 and self.status_msg_x > 0.5 :  # 장애물이 같은 차선에 있을 때
                                while (self.status_msg_heading <= 123):
                                    print("좌회전 중")
                                    print(self.obstacle_dynamic_static_y)
                                    self.curved_index_at_static -= 15
                                    self.center_index = self.curved_index_at_static
                                    
                                    # if self.center_index < 100:
                                    #     self.center_index = 100

                                    self.steer_msg.data = 0.5 + (self.center_index - self.standard_line) * 1/640
                                    self.motor_msg.data = 1.5 * 300

                                    self.publishMotorSteerMsg(self.motor_msg, self.steer_msg)

                                self.curved_index_at_static = 320

                                while (95 <= self.status_msg_heading):
                                    print("우회전으로 복귀 중")
                                    self.curved_index_at_static += 15
                                    self.center_index = self.curved_index_at_static
                            ...
                            if -0.2 < self.obstacle_dynamic_static_y < 0.2 and self.status_msg_x > 0.5 :  # 장애물이 같은 차선에 있을 때
                                while (self.status_msg_heading >= 57):
                                    print("우회전 중")
                                    self.curved_index_at_static += 15
                                    self.center_index = self.curved_index_at_static
                                ...
                                while (85 >= self.status_msg_heading):
                                    print("좌회전으로 복귀 중")
                                    self.curved_index_at_static -= 15
                                    self.center_index = self.curved_index_at_static
                                ...
                                elif self.status_msg_heading < 89:
                                    self.curved_index_at_static -= 2
                                    self.center_index = self.curved_index_at_static
                                elif self.status_msg_heading > 91:
                                    self.curved_index_at_static += 2
                                    self.center_index = self.curved_index_at_static
                    ...
                    # 장애물을 지나갔다는 판단
                    if self.obstacle_dynamic_static_x < 0.5 and not (self.obstacle_dynamic_static_x == 0 and self.obstacle_dynamic_static_y == 0):
                        self.finish_detection = False
                        self.finish_second_detection = False
                        self.obstacle_y_list = []
                        self.dynamic_obstacle_y_list = []
                ...
                # ########################################## 4 4 4 4 4 ##########################################
                    ...
                    if self.status_msg_x >= 12 and self.status_msg_y < 4.7:
                    ...
                    elif self.status_msg_x >= 12 and self.status_msg_y >= 4.7:
                        self.is_slidewindow = True
                        ...
                        elif self.status_msg_y <= 4.5:
                            try:
                                # both line
                                if len(self.left_indices) != 0 and len(self.right_indices) != 0:
                                    self.center_index = (self.left_indices[-1] + self.right_indices[0])//2

                                
                                # left line
                                elif len(self.left_indices) != 0 and len(self.right_indices) == 0:
                                    # center_left_index = (self.left_indices[0] + self.left_indices[-1])//2
                                    self.center_index = self.left_indices[0] + 25 # 0
                                
                                # right line
                                elif len(self.left_indices) == 0 and len(self.right_indices) != 0:
                                    center_right_index = (self.right_indices[0] + self.right_indices[-1])//2
                                    self.center_index = center_right_index
