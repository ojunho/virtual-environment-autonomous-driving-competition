            ...
            # if len(self.img) != 0:
                ...
                # ########################################## 3 3 3 3 3 ##########################################
                    ...
                    if self.is_static:
                        if self.finish_detection == True:
                            self.speed = 4
                            # print("장애물 x : ",self.obstacle_dynamic_static_x)
                            # print("장애물 y: ",self.obstacle_dynamic_static_y)
                ...
                ########################################## 6 6 6 6 6 ##########################################
                    ...
                    if self.is_curved_finished_at_six == False and self.is_green_left_light == True:
                        ...
                        if self.is_traffic_passed == True:
                            # print("커브중")
                            if (-105 <= self.status_msg_heading <= -90):
                                self.is_curved_finished_at_six = True
                            
                            # if self.is_traffic_light == False:
                            self.curved_index_at_six -= 9 # 8

                            self.center_index = self.curved_index_at_six
                            if self.center_index < 125: # 95
                                self.center_index = 125

                        # 신호등에서 멈춘 후에 진행했을 경우
                        else:
                            # print("커브중")
                            # print(self.center_index)
                            if (-107.5 <= self.status_msg_heading <= -90):
                                self.is_curved_finished_at_six = True
                            
                            # if self.is_traffic_light == False:
                            self.curved_index_at_six -= 6.5 #4


                            self.center_index = self.curved_index_at_six
                            if self.center_index < 105:  # 85
                                self.center_index = 105

                    else:
                        self.is_slidewindow = True
                        self.selet_line = "Left"
                ...
                ########################################## 7 7 7 7 7 ##########################################
                    ...
                    if self.is_curved_finished_at_seven == False:
                        ...
                        self.curved_index_at_seven += 18 # 6
                        self.center_index = self.curved_index_at_seven
                        if self.center_index > 605: # 550
                            self.center_index = 605
