...
void cfgCallback(obstacle_detection::hyper_parameter_rotaryConfig &config, int32_t level) {
...
void visualizeObstacle() {
    ...
    for (int i = 0; i < obstacleInfoMsg.obstacleCounts; i++) {
        if (xMinBoundingBox <= obstacleInfoMsg.lengthX[i] && obstacleInfoMsg.lengthX[i] <= xMaxBoundingBox &&
            ...
            obstacleShape.color.r = 0.0;
            obstacleShape.color.g = 0.0;
            obstacleShape.color.b = 1.0;
...
int main (int argc, char** argv) {
    // Initialize ROS
    ros::init (argc, argv, "obstacle_detection_rotary");
    ros::NodeHandle nh;
    
    dynamic_reconfigure::Server<obstacle_detection::hyper_parameter_rotaryConfig> server;
    dynamic_reconfigure::Server<obstacle_detection::hyper_parameter_rotaryConfig>::CallbackType f;
    ...
    pubObstacleInfo = nh.advertise<obstacle_detection::ObstacleInfo>("/obstacle_info_rotary", 1);
    pubObstacleShapeArray = nh.advertise<visualization_msgs::MarkerArray>("/obstacle_marker_rotary", 1);
    pubNearestObstacleInfo = nh.advertise<obstacle_detection::NearestObstacleInfo>("/nearest_obstacle_info_rotary", 1);
