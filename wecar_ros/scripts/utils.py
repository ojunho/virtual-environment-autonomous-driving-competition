...
class purePursuit :
    ...
    #         # 차량이 멀어질 수록 
    #         # X는 값이 작아지고, Y는 값이 커짐.
    #         # print(f"X: {round(rotated_point.x, 2)}")
    #         # print(f"Y: {round(rotated_point.y, 2)}")
            
            
    #         # print(f"yaw: {self.vehicle_yaw*180/pi}")
    #         self.lfd = 1.3
    #         # vel -> lfd 
    #         # 8 -> 1.5
    ...
    #             # print(f"dis: {dis}, lfd: {self.lfd}")
    #             # 왜 밑의 조건문에 조금만 걸리는지, -> lfd는 4로 거의 같은데, dis: 4보다 계속 작음.
    ...
    #             # 밑의 코드의 의미: 
    #             if dis >= self.lfd :
    #                 # print("theta:", atan2(rotated_point.y, rotated_point.y))   
    #                 # self.lfd = 1
    ...
    #     return self.steering, self.forward_point.x, self.forward_point.y
